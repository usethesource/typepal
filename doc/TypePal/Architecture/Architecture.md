...
first a new ((TypePal:Collector)) is created (using `newCollector`) and that enables the author of the type checker
...
This is done by creating a new ((TypePal:Solver)) (using `newSolver`) and running it on the given `TModel`.
